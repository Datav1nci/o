...
/** Tiny helper to replicate your working parallax pattern */
function ParallaxBox({
  ...
  src: string;
  alt: string;
  /** e.g. 'h-[52vh] min-h-[320px]' OR 'aspect-video' OR custom fixed height */
  heightClass: string;
}) {
  return (
    <div
      ...
      aria-label={alt}
      className={cn(
        "relative overflow-hidden rounded-2xl border border-gray-200 shadow-sm dark:border-gray-800",
        heightClass,
      )}
    >
      {/* Parallax layer */}
      ...
      />
      {/* Subtle overlay border to match card look */}
    ...
    <main className="container mx-auto max-w-7xl px-4 py-8">
      ...
      <nav aria-label="Fil d’Ariane" className="mb-6 text-sm text-slate-600">
        <ol className="flex flex-wrap items-center gap-1">
          ...
          <li className="mx-1 opacity-60">/</li>
          <li><span className="opacity-80">Chlore</span></li>
          <li className="mx-1 opacity-60">/</li>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        {/* Left visual with parallax — using your working technique */}
        <ParallaxBox
          src="/images/filter_devices_hero.webp"
          alt="Visuel Le Grand (espace réservé)"
          // approximate portrait poster look
        ...
        <div className="px-1">
          ...
          <p className="mt-3 max-w-prose text-slate-700 dark:text-slate-300">
            ...
            solides, ainsi que la réduction des substances altérant la qualité de l’eau :
            mauvais goûts et odeurs, chlore, COV (composants organiques volatils).
          </p>
